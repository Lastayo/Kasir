...
exports.findTransaksi = async (request, response) => {
    ...
    let cari = await sequelize.query(`SELECT transaksi.id_transaksi, transaksi.nama_pelanggan, transaksi.tgl_transaksi, transaksi.status, detail_transaksi.harga, menu.nama_menu FROM transaksi JOIN detail_transaksi ON transaksi.id_transaksi = detail_transaksi.id_transaksi JOIN menu ON detail_transaksi.id_menu = menu.id_menu WHERE transaksi.nama_pelanggan = '${keyword}';`)
